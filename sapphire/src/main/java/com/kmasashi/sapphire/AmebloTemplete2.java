...
public class AmebloTemplete2 extends Ameblo {

	public AmebloTemplete2(String blog) {
		super(blog);
	}
	...
	protected Map<String, String> getArticle(String article, File outFile) {
		...
		Pattern p = Pattern.compile("<a class=\"\" href=\"/" + this.BLOG + "/(.*\\.html)\">(.*)</a>");
		...
		String articleTheme = document.select(".skin-entryThemes").text();
		System.out.println("Theme:" + articleTheme);

		String articleText = document.select(".skin-entryBody").text();
//			System.out.println(articleText.html());
	...
	protected Map<String, Integer> getMonthlyArticleCountList(String year) {
		...
		Elements listContentsArea = document.select(".skin-monthlyNav");
...
//			System.out.println(contentTitle.html());
			...
			Elements contentTitleArea = document.select("[data-uranus-component=entryItemTitle]");
